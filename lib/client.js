...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".thms_segRow{display:flex;margin-top:2px}",
		...
		const zh = {
			...
			settingsDescription: "新会话页面 Token 用量热力图的配色与默认视图。",
			settingsScheme: "配色方案",
		...
		const en = {
			...
			settingsDescription: "Colors and default view of the token heatmap on the new-session screen.",
		 ...
		 * badge, discard/save footer). Fields: 配色方案 and 默认视图 — the 0.1.x
		 * 显示热力图 master switch was dropped (the card is always shown on
		 * the hero screen), so its stored `enabled` key is no longer read or
		 * written. The legacy loopback config endpoint remains as a
		 * back-compat API, but the card no longer uses it.
		...
		function TokenHeatmapSettingsCard({ t }) {
			...
			react.useEffect(() => {
				...
				setLoaded({ colorScheme: config.colorScheme, defaultView: config.defaultView });
				setDraft({ colorScheme: config.colorScheme, defaultView: config.defaultView });
			}, [config, loaded]);

			const dirty = loaded !== null && draft !== null && (draft.colorScheme !== loaded.colorScheme || draft.defaultView !== loaded.defaultView);
			const save = react.useCallback(async () => {
				if (draft === null || saving) return;
				setSaving(true);
				setSaveFailed(false);
				try {
					await configStore.set({ colorScheme: draft.colorScheme, defaultView: draft.defaultView });
					...
					if (confirmed.status === "ready") {
						setLoaded({ colorScheme: confirmed.colorScheme, defaultView: confirmed.defaultView });
						setDraft({ colorScheme: confirmed.colorScheme, defaultView: confirmed.defaultView });
					}
				} catch {
					setSaveFailed(true);
				} finally {
