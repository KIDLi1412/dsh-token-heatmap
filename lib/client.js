...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".thm_viewNav{border:1px solid var(--dsw-alias-border-l2);border-radius:6px;align-items:center;display:inline-flex;overflow:hidden}",
			".thm_viewBtn{cursor:pointer;color:var(--dsw-alias-label-tertiary);background:0 0;border:none;padding:1px 8px;font:inherit;font-size:11px;line-height:16px}",
			...
			".thm_monthCal{width:max-content;display:flex;flex-direction:column;gap:4px}",
			".thm_weekHead{margin-left:20px;display:flex;gap:4px}",
			".thm_weekHeadCell{color:var(--dsw-alias-label-caption);width:40px;flex:none;font-size:9px;line-height:12px;text-align:center}",
			".thm_week{margin-left:20px;display:flex;gap:4px}",
			".thm_day{box-sizing:border-box;width:40px;height:40px;border-radius:5px;flex-direction:column;align-items:center;justify-content:center;gap:1px;display:flex}",
			...
			".thm_day[data-level=\"3\"] .thm_dayNum,.thm_day[data-level=\"4\"] .thm_dayNum{color:#fff}",
			".thm_day[data-level=\"3\"] .thm_dayTokens,.thm_day[data-level=\"4\"] .thm_dayTokens{color:rgba(255,255,255,.72)}",
			...
			".thms_segRow{display:flex}",
			".thms_seg{display:inline-flex;overflow:hidden;border:1px solid var(--dsw-alias-border-l2);border-radius:8px}",
			".thms_segBtn{appearance:none;font:inherit;cursor:pointer;color:var(--dsw-alias-label-secondary);background:0 0;border:0;padding:4px 18px;font-size:13px;line-height:1.5}",
			".thms_segBtn:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover)}",
			".thms_segBtn[data-active=true]{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover);font-weight:600}",
			".thms_segBtn:disabled{opacity:.5;cursor:default}",
		...
		const S = {
			...
			settingsSegRow: "thms_segRow",
			settingsSeg: "thms_seg",
			settingsSegBtn: "thms_segBtn",
		...
		const zh = {
			...
			settingsDefaultViewHint: "新会话页面首次打开时显示的视图（可在热力图右上角随时切换）。",
			settingsGreen: "绿色",
			settingsBlue: "蓝色",
			settingsOrange: "橙色",
			settingsRed: "红色",
		...
		const en = {
			...
			settingsDefaultViewHint: "The view the new-session screen opens with (switchable anytime from the card header).",
			settingsGreen: "Green",
			settingsBlue: "Blue",
		...
		/**
		 * Display preferences (enabled + colorScheme + defaultView), persisted
		 * through the Host `token-heatmap` settings namespace (settings.yaml)
		 * via the bound settings scope — NOT the legacy loopback config
		 * endpoint. The heatmap and the settings card read the store through
		 * useSyncExternalStore; while the scope is loading or unavailable the
		 * store falls back to the defaults (enabled / green / year view), so
		 * the UI never blocks on the settings transport.
		 */
		const CONFIG_DEFAULTS = { enabled: true, colorScheme: "green", defaultView: "year" };
		 ...
		 * `{ status, enabled, colorScheme, defaultView }` derived from the
		 ...
		 * snapshot only changes after the Host confirms. `reload()` re-reads
		 * from the Host; `dispose()` removes the subscription.
		 */
		...
		/**
		 * Build the calendar grid for ONE MONTH (the ‹年›/‹月› toggle's month
		...
		function TokenHeatmap({ session, input, t }) {
			...
			// Active view mode, seeded from the persisted default (年/月 toggle
			// next to the refresh button; a manual toggle wins for this mount).
		...
		function TokenHeatmapSettingsCard({ t }) {
			...
			react.useEffect(() => {
				...
				setLoaded({ enabled: config.enabled, colorScheme: config.colorScheme, defaultView: config.defaultView });
				setDraft({ enabled: config.enabled, colorScheme: config.colorScheme, defaultView: config.defaultView });
			}, [config, loaded]);

			const dirty = loaded !== null && draft !== null && (draft.enabled !== loaded.enabled || draft.colorScheme !== loaded.colorScheme || draft.defaultView !== loaded.defaultView);
			const save = react.useCallback(async () => {
				if (draft === null || saving) return;
				setSaving(true);
				setSaveFailed(false);
				try {
					await configStore.set({ enabled: draft.enabled, colorScheme: draft.colorScheme, defaultView: draft.defaultView });
					...
					if (confirmed.status === "ready") {
						setLoaded({ enabled: confirmed.enabled, colorScheme: confirmed.colorScheme, defaultView: confirmed.defaultView });
						setDraft({ enabled: confirmed.enabled, colorScheme: confirmed.colorScheme, defaultView: confirmed.defaultView });
					}
				} catch {
					setSaveFailed(true);
				} finally {
			...
			const chooseView = (view) => {
				if (draft !== null) setDraft({ ...draft, defaultView: view });
			};
				...
				bodyContent = react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, {
					children: [
						...
						react_jsx_runtime.jsxs("div", {
							className: S.settingsField,
							children: [
								react_jsx_runtime.jsx("span", { className: S.settingsFieldLabel, children: dict("settingsDefaultView") }),
								react_jsx_runtime.jsx("div", {
									className: S.settingsSegRow,
									children: react_jsx_runtime.jsx("div", {
										className: S.settingsSeg,
										role: "group",
										"aria-label": dict("settingsDefaultView"),
										children: VIEW_MODES.map((mode) => react_jsx_runtime.jsx("button", {
											type: "button",
											className: S.settingsSegBtn,
											"data-active": draft.defaultView === mode,
											"aria-pressed": draft.defaultView === mode,
											disabled: saving,
