...
//#region usage ledger
/**
 * DSH built-in usage ledger location — the same per-day/per-provider/per-model
 * aggregate the 设置 → 使用统计 card reads (`<DSH_HOME>/dsh-usage/usage-ledger.json`).
 * The ledger is complete and real-time, so it is the heatmap's PRIMARY source;
 * the incremental session-event fold below is a fallback for DSH builds that
 * do not maintain a ledger.
 */
function ledgerPath() {
	const home = process.env.DSH_HOME ?? join(homedir(), ".dsh");
...
/**
 * Read and validate the DSH built-in usage ledger. Returns the parsed ledger
 * when it is present and a supported version, or null when it is absent,
 * corrupt, or an unsupported version — callers fall back to the session-event
 * fold. The ledger is written atomically (temp + rename) by DSH, so a read
...
async function loadLedger() {
	try {
		...
		const parsed = JSON.parse(raw);
		if (parsed !== null && typeof parsed === "object" && parsed.version === 1 && parsed.days !== null && typeof parsed.days === "object") {
			return parsed;
		}
	} catch {
		/* ledger absent or corrupt — fall back to the session-event fold */
	}
	return null;
}
//#endregion
...
export async function collectUsage(ctx) {
	return withLock(async () => {
		// Primary source: the DSH built-in usage ledger (设置 → 使用统计). It is
		// complete and real-time, so the heatmap stays exact across process
		// restarts. The session-event fold below cannot enumerate persisted
		// sessions on DSH 0.1.2+ rc.1 (sessionPersistence no longer exposes
		// list/listSnapshots) and only folds live sessions while the hero screen
		// is mounted (the client polls the usage endpoint only there), which
		// silently drops both same-day usage from sessions that ended off-hero
		// and all pre-restart history. The ledger has neither gap.
		const ledger = await loadLedger();
		if (ledger !== null) {
			return renderLedger(ledger, Date.now());
		}
		// Fallback: incremental session-event fold (pre-0.1.2 or no ledger).
