 ...
 * boundaries without replaying the whole log. `skipUntil` is the fork-inherited
 * prefix length (0 for a non-forked session): those events belong to the fork
 * parent, so folding starts there to avoid counting them twice.
 */
...
/**
 * Render the optional @linxin666/dsh-usage ledger
 * (`<DSH_HOME>/dsh-usage/usage-ledger.json`) into the same wire shape as
 * renderUsage. The ledger is the complete, real-time, per-day/per-provider/
 * per-model token aggregate that the @linxin666/dsh-usage 设置 → 使用统计 card
 * reads; when present, collectUsage serves it as an enhancement over the
 * session/event fold to recover full history. The file only exists when that
 * third-party plugin is installed and enabled — it is NOT a DSH contract.
 *
 * @linxin666/dsh-usage prunes days older than retainDays (default 180,
 * max 730), so while the ledger is the source the all-time total and older
 * calendar years are bounded by that setting.
 *
 * Token口径与 renderUsage 一致：input + output + cacheRead + cacheWrite
 * (reasoningTokens 是思维链长度，不计入主用量).
 *
