...
- 🔀 **年 / 月切换**：标题行**最右侧**（刷新按钮右边）的分段按钮即时切换视图；默认视图可在 设置 → 插件 → 插件配置 里选（见下）。
