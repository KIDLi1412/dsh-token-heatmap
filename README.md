...
- **0.1.7（fork 会话双计修复）**：DSH 的 fork 子会话（header `isSeeded=true`）日志以父会话事件的完整复制开头，其前 `inheritedEventCount` 个事件是**父会话**的 usage（父会话折叠时已计入）。此前折叠从 seq 0 读整份日志，导致同一批 token 被计两次——实测 2026-09-14 由 8.34 亿虚增到 13.15 亿。现折叠从 fork 切点开始：in-process 用官方 `session.inheritedEventCount`，持久化日志定位最后一个 `data.inherited === true` 的 `session/end-seed` 事件并取其 seq + 1。`session/end-seed` 但 `inherited` 未标记的是 compaction 边界、`isSeeded=false` 的 resume 会话不是 fork（其 seed 是自己的历史），二者都必须全折——只有 `isSeeded=true` 才跳过。缓存版本提升以强制重折已污染的历史。
